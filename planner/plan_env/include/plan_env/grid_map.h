...
/*修改，以前支持的点云需要在定位算法那边发出，当前修改只是lidar系，或者body系下的点云*/

#define logit(x) (log((x) / (1 - (x))))

using namespace std;

enum  class CloudCoord: int{LOCAL=1,BODY=2,MAP=3}; //点云坐标系类型
...
private:
  ...
  void cloudCallback(const sensor_msgs::PointCloud2ConstPtr& img,const CloudCoord& cloud_coord);
  ...
  SynchronizerImagePose sync_image_pose_indep_;
