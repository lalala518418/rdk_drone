...
namespace ego_planner
{
  ...
  void EGOReplanFSM::waypointCallback(const nav_msgs::PathConstPtr &msg)
  {
    ...
    trigger_ = true;
    init_pt_ = odom_pos_;
    std::cout << "ego_goal : " << msg->poses[0].pose.position.x << "," << msg->poses[0].pose.position.y << std::endl;

    std::cout << "odom_pos : " << odom_pos_ << std::endl;
  ...
  bool EGOReplanFSM::callReboundReplan(bool flag_use_poly_init, bool flag_randomPolyTraj)
  {
    ...
    cout << "start: " << start_pt_.transpose() << ", " << start_vel_.transpose() << "\ngoal:" << local_target_pt_.transpose() << ", " << local_target_vel_.transpose()
         << endl;

    std::cout << "replan odom_pose " << odom_pos_ << std::endl;
